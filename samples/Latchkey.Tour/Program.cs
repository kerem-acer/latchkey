...
string demoDir = Path.Combine(Path.GetTempPath(), "latchkey-tour-" + Guid.NewGuid().ToString("N"));
...
try
{
    ...
    bool nativeWorks = Latchkey.Latchkey.VerifyPersistence(service);
    Console.WriteLine(nativeWorks
        ? "A native OS credential store is available — in your app, Auto would use it."
        : "No native store here (headless/container) — Auto would throw; we use an explicit backend.");
    ...
    ILatchkey store = LatchkeyFactory.Create(new LatchkeyOptions
    {
        ...
        BackendOptions = [FileBackendOption.Default with { Path = demoDir }],
    });
    ...
    byte[] secret = [0x00, 0x01, 0x02, 0xFF, 0x00];
    store.Set("raw", secret);
    byte[]? read = store.GetBytes("raw");
    Console.WriteLine($"raw bytes round-trip -> {read is not null && read.AsSpan().SequenceEqual(secret)}");
    if (read is not null)
    {
        try { /* use the secret here */ }
        finally { CryptographicOperations.ZeroMemory(read); } // wipe it when you're done
    }
    ...
    ILatchkey mapped = LatchkeyFactory.Create(new LatchkeyOptions
    {
        ServiceName = service,
    ...
    ILatchkey custom = LatchkeyFactory.Create(new LatchkeyOptions
    {
        ServiceName = service,
    ...
    ILatchkey injected = provider.GetRequiredService<ILatchkey>();
...
finally
{
    ...
    try { Directory.Delete(demoDir, recursive: true); }
...
file sealed class DictionaryBackend : ISecretBackend
{
    ...
    public byte[]? Retrieve(string service, string key) =>
        _store.TryGetValue((service, key), out byte[]? value) ? value : null;
