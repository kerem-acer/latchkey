...
/// </summary>
internal sealed class InMemoryBackend : ISecretBackend
{
    private readonly ConcurrentDictionary<(string Service, string Key), byte[]> _store = new();
    ...
    public byte[]? Retrieve(string service, string key)
    {
        ...
        return _store.TryGetValue((service, key), out var value) ? (byte[])value.Clone() : null;
    }
